...
public class ImplRenderContext3D implements RenderContext3D {
    ...
    private ConsumerType lastConsumerType = null;
    ...
    @Override
    public void vertex(float x, float y, float z, int rgb) {
        VertexConsumer vertexConsumer = boxConsumer();
        vertexConsumer.vertex(mcContext.matrixStack().peek(), x, y, z).color(rgb);
    }
    ...
    private VertexConsumer boxConsumer(){
        if(lastConsumerType == ConsumerType.BOX) return lastConsumer;

        lastConsumer = mcContext.consumers().getBuffer(RenderLayer.getDebugFilledBox());
        lastConsumerType = ConsumerType.LINE;
    ...
    private VertexConsumer lineConsumer(){
        ...
        lastConsumer = mcContext.consumers().getBuffer(lineLayer);
        lastConsumerType = ConsumerType.LINE;
